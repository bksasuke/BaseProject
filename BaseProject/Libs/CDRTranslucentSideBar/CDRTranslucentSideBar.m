...
- (void)initCDRTranslucentSideBar {
    ...
    [_blurView addGestureRecognizer:self.tapGestureRecognizer];
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
	...
	void (^animations)() = ^{
        _blurView.alpha = kAlphaBlurView;
		if (self.contentView != nil) {
			self.contentView.frame = sideBarFrame;
		}
...
- (void)handleTapGesture:(UITapGestureRecognizer *)recognizer {
    CGPoint location = [recognizer locationInView:_blurView];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)recognizer {
    ...
    if (!self.showFromRight) {
        CGFloat percentOfSubViewVisible =  (self.panStartPoint.x - [recognizer locationInView:self.view].x )/ self.contentView.frame.size.width;
        if ((kAlphaBlurView - percentOfSubViewVisible) > kAlphaBlurView) {
            _blurView.alpha = kAlphaBlurView;
        } else {
            _blurView.alpha = kAlphaBlurView - percentOfSubViewVisible;
        }
    } else {
        CGFloat percentOfSubViewVisible =  ([recognizer locationInView:self.view].x - self.panStartPoint.x )/ self.contentView.frame.size.width;
        if ((kAlphaBlurView - percentOfSubViewVisible) > kAlphaBlurView) {
            _blurView.alpha = kAlphaBlurView;
        } else {
            _blurView.alpha = kAlphaBlurView - percentOfSubViewVisible;
...
- (void)handlePanGestureToShow:(UIPanGestureRecognizer *)recognizer inView:(UIView *)view {
    ...
    if (!self.showFromRight) {
        CGFloat percentOfSubViewVisible =  ([recognizer locationInView:view].x - self.panStartPoint.x)/ self.contentView.frame.size.width;
        _blurView.alpha = MIN(kAlphaBlurView, percentOfSubViewVisible);
    } else {
        CGFloat percentOfSubViewVisible =  (self.panStartPoint.x - [recognizer locationInView:view].x)/ self.contentView.frame.size.width;
        _blurView.alpha = MIN(kAlphaBlurView, percentOfSubViewVisible);
    }
...
- (void)setContentViewInSideBar:(UIView *)contentView {
    ...
    [self.view insertSubview:_blurView belowSubview:self.contentView];
